...
def init_mesh_renderer(image_size=512, dist=3.5, elev=90, azim=90, camera='0', device='cuda:0'):
    ...
    if camera == '0':
        # for vox orientation
        # dist, elev, azim = 2.0, 20, 20+270 # shapenet
...
def sdf_to_mesh(sdf, level=0.02, color=None, render_all=False, centroid=None, m=None):
    ...
    for i in range(nimg_to_render):
        ...
        verts_i, faces_i = mcubes.marching_cubes(sdf_i, level)
        verts_i = verts_i / n_cell - .5
        # print(f'[*] before normalize: {verts_i.min()}, {verts_i.max()}')
...
def replace_zeros_with_valid_point(verts):
    # verts: [B, N, 3]
    replaced = verts.clone()
    for b in range(verts.size(0)):
        sample = verts[b]
        # 找出非零点
        non_zero_mask = ~(sample == 0).all(dim=1)
        if non_zero_mask.any():
            # 取第一个非零点来替换
            replace_value = sample[non_zero_mask][0]
            replaced[b][~non_zero_mask] = replace_value
    return replaced
...
def render_pcd(renderer, verts, color=[1, 1, 1], alpha=False):
    ...
    verts = replace_zeros_with_valid_point(verts)
...
def render_sdf(mesh_renderer, sdf, level=0.02, color=None, render_imsize=256, render_all=True):
    """ 
        shape of sdf:
        - bs, 1, nC, nC, nC
